...
namespace RimPlas
{
	...
	public class PlaceWorker_RPElectroliser : PlaceWorker
	{
		...
		{
			foreach (IntVec3 item in CompPowerPlant_RPElectroliser.GroundCells(loc, rot))
			{
				if (!map.terrainGrid.TerrainAt(item).affordances.Contains(TerrainAffordanceDefOf.Medium))
				{
					return new AcceptanceReport("TerrainCannotSupport_TerrainAffordance".Translate(checkingDef, TerrainAffordanceDefOf.Medium));
				}
			}
		...
		{
			foreach (IntVec3 item in CompPowerPlant_RPElectroliser.WaterUseCells(loc, rot))
			{
		...
		{
			GenDraw.DrawFieldEdges(CompPowerPlant_RPElectroliser.GroundCells(loc, rot).ToList<IntVec3>(), Color.white);
			...
			GenDraw.DrawFieldEdges(CompPowerPlant_RPElectroliser.WaterUseCells(loc, rot).ToList<IntVec3>(), color);
		}
